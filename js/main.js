document.addEventListener('DOMContentLoaded', () => {
  ...
  const adjustMenu = init => {
    const getAllWidth = ele => Array.from(ele || []).reduce((width, i) => width + (i ? i.offsetWidth : 0), 0)

    if (init) {
      const $blogInfoLink = document.querySelector('#blog-info > a')
      const $menus = document.getElementById('menus')
      $nav = document.getElementById('nav')
      if (!$blogInfoLink || !$menus || !$nav) return
      const blogInfoWidth = getAllWidth($blogInfoLink.children)
